...
The dbt project is configured to work with the Delta Lake outputs generated by Spark. In the current repo, the expected Silver locations are `data/delta/silver/stocks`, `data/delta/silver/news_posts`, and `data/delta/silver/fx_rates`; if the output paths change, update the corresponding external locations in [pulse_dbt/models/sources.yml](pulse_dbt/models/sources.yml).
